...
struct CPUMesh
{
    ...
    // Lightmap UV1 — one vec2 per vertex, parallel to vertexData.
    // Generated by xatlas at import time. Empty when no lightmap UVs are available.
...
struct GPUMesh
{
    ...
    // Lightmap UV buffer — bound at vertex binding 1 during GBuffer and baking passes.
    // Null when the mesh has no lightmap UVs (most meshes during normal rendering).
    ...
    static std::shared_ptr<GPUMesh> createFromMesh(const CPUMesh &mesh, core::CommandPool::SharedPtr commandPool = nullptr)
    {
        ...
        // Upload lightmap UVs as a second vertex buffer if present.
