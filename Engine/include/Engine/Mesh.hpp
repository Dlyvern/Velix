...
struct GPUMesh
{
    ...
    core::Buffer::SharedPtr lightmapUVBuffer{nullptr};
    ...
    int32_t unifiedVertexOffset{INVALID_VERTEX_OFFSET};
    uint32_t unifiedFirstIndex{0};
    ...
    static std::shared_ptr<GPUMesh> createFromMesh(const CPUMesh &mesh, core::CommandPool::SharedPtr commandPool = nullptr)
    {
        ...
        {
            const VkDeviceSize uvSize = sizeof(glm::vec2) * mesh.lightmapUVs.size();

            auto cmdBuf = core::CommandBuffer::createShared(*core::VulkanContext::getContext()->getGraphicsCommandPool());
            cmdBuf->begin();

            auto staging = core::Buffer::createShared(uvSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, core::memory::MemoryUsage::CPU_TO_GPU);
            staging->upload(mesh.lightmapUVs.data(), uvSize);

            auto gpuUVBuf = core::Buffer::createShared(uvSize,
                VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                core::memory::MemoryUsage::GPU_ONLY);

            utilities::BufferUtilities::copyBuffer(*staging, *gpuUVBuf, *cmdBuf, uvSize);
            cmdBuf->end();

            utilities::AsyncGpuUpload::submit(cmdBuf, core::VulkanContext::getContext()->getGraphicsQueue(), {staging});
            gpuMesh->lightmapUVBuffer = gpuUVBuf;
        }
