...
FXAARenderGraphPass::FXAARenderGraphPass(std::vector<RGPResourceHandler> &inputHandlers)
    : m_inputHandlers(inputHandlers)
{
    setDebugName("FXAA render graph pass");
    m_clearValues[0].color = {0.f, 0.f, 0.f, 1.f};
    setExtent(core::VulkanContext::getContext()->getSwapchain()->getExtent());
    outputs.color.setOwner(this);
}

void FXAARenderGraphPass::setup(renderGraph::RGPResourcesBuilder &builder)
{
    ...
    {
        outDesc.setDebugName("__ELIX_FXAA_" + std::to_string(i) + "__");
    ...
    m_pipelineLayout = core::PipelineLayout::createShared(
        ...
        std::vector<VkPushConstantRange>{PushConstant<FXAApc>::getRange(VK_SHADER_STAGE_FRAGMENT_BIT)});
...
void FXAARenderGraphPass::compile(renderGraph::RGPResourcesStorage &storage)
{
    const uint32_t imageCount = core::VulkanContext::getContext()->getSwapchain()->getImageCount();
...
std::vector<IRenderGraphPass::RenderPassExecution> FXAARenderGraphPass::getRenderPassExecutions(const RenderGraphPassContext &renderContext) const
{
...
void FXAARenderGraphPass::setExtent(VkExtent2D extent)
{
...
void FXAARenderGraphPass::freeResources()
{
