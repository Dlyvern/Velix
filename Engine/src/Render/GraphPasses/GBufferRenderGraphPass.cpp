                                    ...
                                    const RenderGraphPassContext &renderContext)
{
    ...
    for (const auto &batch : data.drawBatches)
    {
        ...
        key.shader = batch.skinned ? ShaderId::GBufferSkinned : ShaderId::GBufferStatic;
        ...
        key.rasterizationSamples = rasterizationSamples;
        key.gbufferOutputMode = isObjectOnlyExecution ? GBufferOutputMode::ObjectOnly : GBufferOutputMode::Full;
        ...
        vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(ModelPushConstant),
                           &modelPushConstant);

        std::array<VkDescriptorSet, 3> descriptorSets{
            data.cameraDescriptorSet,                                 // set 0: camera & light
            batch.material->getDescriptorSet(renderContext.currentFrame), // set 1: material
            data.perObjectDescriptorSet                               // set 2 : per object(bones + instances)
        };
...
std::vector<IRenderGraphPass::RenderPassExecution> GBufferRenderGraphPass::getRenderPassExecutions(const RenderGraphPassContext &renderContext) const
{
    ...
    mainExecution.colorsRenderingItems = {normalColor, albedoColor, materialColor, objectColor};
    ...
    objectPassDepth.clearValue = m_clearValues[4];

    objectExecution.colorsRenderingItems = {objectPassNormal, objectPassAlbedo, objectPassMaterial, objectPassColor};
