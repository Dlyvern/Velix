...
RTShadowDenoiseRenderGraphPass::RTShadowDenoiseRenderGraphPass(std::vector<RGPResourceHandler> &rawShadowHandlers,
                                                               std::vector<RGPResourceHandler> &normalHandlers,
                                                               RGPResourceHandler &depthHandler)
    : m_rawShadowHandlers(rawShadowHandlers),
...
bool RTShadowDenoiseRenderGraphPass::shouldDenoise() const
{
    ...
    if (!settings.enableRayTracing || !settings.enableRTShadows || !context || m_computePipeline == VK_NULL_HANDLE)
        return false;
    if (settings.rayTracingMode == RenderQualitySettings::RayTracingMode::Pipeline)
        return context->hasRayTracingPipelineSupport();
    if (settings.rayTracingMode == RenderQualitySettings::RayTracingMode::RayQuery)
        return context->hasRayQuerySupport();
    return false;
}

void RTShadowDenoiseRenderGraphPass::setup(renderGraph::RGPResourcesBuilder &builder)
{
    const uint32_t imageCount = core::VulkanContext::getContext()->getSwapchain()->getImageCount();

    for (uint32_t i = 0; i < imageCount; ++i)
    {
        builder.read(m_rawShadowHandlers[i], RGPTextureUsage::SAMPLED);
    ...
    for (uint32_t i = 0; i < imageCount; ++i)
    {
        outputDescription.setDebugName("__ELIX_RT_SHADOWS_DENOISED_" + std::to_string(i) + "__");
    ...
    m_pipelineLayout = core::PipelineLayout::createShared(
        ...
        std::vector<VkPushConstantRange>{
            PushConstant<RTShadowDenoisePC>::getRange(VK_SHADER_STAGE_COMPUTE_BIT)});

    m_shadowSampler = core::Sampler::createShared(VK_FILTER_NEAREST, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_BORDER_COLOR_INT_OPAQUE_BLACK);
...
void RTShadowDenoiseRenderGraphPass::compile(renderGraph::RGPResourcesStorage &storage)
{
    const uint32_t imageCount = core::VulkanContext::getContext()->getSwapchain()->getImageCount();
    ...
    for (uint32_t i = 0; i < imageCount; ++i)
    {
        const auto *rawShadowTarget = storage.getTexture(m_rawShadowHandlers[i]);
        ...
        if (!m_descriptorSetsInitialized)
        {
            m_descriptorSets[i] = DescriptorSetBuilder::begin()
                                      .addImage(rawShadowTarget->vkImageView(), m_shadowSampler, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0)
...
void RTShadowDenoiseRenderGraphPass::record(core::CommandBuffer::SharedPtr commandBuffer,
                                            ...
                                            const RenderGraphPassContext &renderContext)
{
    ...
    if (data.drawBatches.empty())
    {
        ...
        clearRange.baseArrayLayer = 0;
        clearRange.layerCount     = kMaxShadowLights;
        vkCmdClearColorImage(commandBuffer->vk(), outputTarget->getImage()->vk(), VK_IMAGE_LAYOUT_GENERAL, &clearColor, 1, &clearRange);
        return;
    }

    RTShadowDenoisePC pushConstants{};
    ...
    pushConstants.params1 = glm::vec4(
        0.75f,
        0.0f,
        static_cast<float>(std::min(renderContext.activeRTShadowLayerCount, kMaxShadowLights)),
        0.0f);
...
std::vector<IRenderGraphPass::RenderPassExecution> RTShadowDenoiseRenderGraphPass::getRenderPassExecutions(
...
void RTShadowDenoiseRenderGraphPass::cleanup()
{
    destroyComputePipeline();
}

void RTShadowDenoiseRenderGraphPass::createComputePipeline()
{
    destroyComputePipeline();
    ...
    m_computeShader.loadFromFile("./resources/shaders/rt_shadow_denoise.comp.spv", core::ShaderStage::COMPUTE);
    ...
    if (vkCreateComputePipelines(context->getDevice(), VK_NULL_HANDLE, 1, &pipelineCreateInfo, nullptr, &m_computePipeline) != VK_SUCCESS)
        throw std::runtime_error("Failed to create RT shadow denoise compute pipeline");
}

void RTShadowDenoiseRenderGraphPass::destroyComputePipeline()
{
    auto context = core::VulkanContext::getContext();
