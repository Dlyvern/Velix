...
float calculateDirectionalLightShadow(vec3 worldPos, vec3 lightDirWorld, vec3 normalWorld)
{
    vec4 posLS = lightSpaceData.lightSpaceMatrix * vec4(worldPos, 1.0);
    ...
    for (int y = -1; y <= 1; ++y)
    {
        float pcfDepth = texture(shadowMap, texCoords + vec2(x, y) * texelSize).r;
...
void main()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (lightType == DIRECTIONAL_LIGHT_TYPE)
        {
            ...
            shadow = calculateDirectionalLightShadow(P_world, L_world, N_world);
        }
